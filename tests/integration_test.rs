...
#[tokio::test]
async fn test_integration_zkvm() {
    // Initialize enhanced logging with structured output, filtering out verbose internal logs
    use tracing_subscriber::filter::EnvFilter;
    ...
    println!("NUM_TEST_QUERIES: {}", NUM_TEST_QUERIES);
    println!(
        "packed_mle_values.total_n_vars: {}",
        packed_mle_values.total_n_vars
    );
    ...
    let result = friveil.verify_evaluation(
        &mut verifier_transcript,
        evaluation_claim.clone(),
        &evaluation_point.clone(),
        &fri_params,
    );

    match result {
        Ok(_) => info!("✅ Evaluation proof verified successfully"),
    ...
    println!("Response: {} {}", data.status(), data.text().await.unwrap());
